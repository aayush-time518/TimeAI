        ...
        <SimulationStage>
            <div className="w-full h-full flex items-center justify-around gap-2 sm:gap-4 px-4 sm:px-8">
                ...
                <div className={`flex flex-col items-center w-1/3 transition-all duration-700 ${step <= 1 ? 'scale-100 opacity-100' : 'scale-90 opacity-40'}`}>
                    <div className="w-16 h-20 sm:w-24 sm:h-32 bg-white rounded-lg shadow-2xl relative overflow-hidden flex flex-col p-2 sm:p-3 gap-1.5 border border-slate-200">
                        {step === 0 && (
                            <div className="absolute inset-0 bg-slate-900/10 backdrop-blur-[1px] flex items-center justify-center z-30">
                                <Loader2 className="w-6 h-6 text-tva-orange animate-spin" />
                            </div>
                        )}
                        <div className="h-2 w-2/3 bg-slate-200 rounded"></div>
                        <div className="h-1.5 w-full bg-slate-100 rounded"></div>
                        <div className={`absolute inset-0 flex flex-col gap-1.5 p-2 transition-all duration-500 bg-white ${step === 1 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                            {Array.from({length: 3}).map((_, i) => (
                                <div key={i} className="h-4 bg-blue-500/10 border border-blue-500/30 rounded-md animate-pulse"></div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="w-1/4 flex flex-col items-center justify-center">
                     <div className={`w-20 h-16 sm:w-28 sm:h-20 border-2 rounded-xl sm:rounded-2xl bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-2 transition-all duration-700 ${step === 2 ? 'border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.2)]' : 'border-slate-800 opacity-20'}`}>
                        {step === 2 ? (
                            <div className="flex gap-0.5 sm:gap-1 h-6 sm:h-8 items-end justify-center">
                                {[5, 9, 7, 10, 6].map((h, i) => (
                                    <div key={i} className="w-1 bg-blue-500 rounded-t-sm" style={{ height: `${h * 10}%`, animation: `bounce ${0.6 + i*0.1}s ease-in-out infinite` }}></div>
                                ))}
                            </div>
                ...
                <div className={`flex flex-col items-center w-1/3 transition-all duration-700 ${step >= 3 ? 'scale-100 opacity-100' : 'scale-90 opacity-20'}`}>
                    <div className={`w-24 h-24 sm:w-36 sm:h-36 border-2 rounded-2xl sm:rounded-3xl flex flex-col items-center justify-center bg-slate-900 transition-all duration-700 relative overflow-hidden ${step === 3 ? 'border-blue-400 shadow-[0_0_30px_rgba(59,130,246,0.3)]' : step === 4 ? 'border-green-500' : 'border-slate-800'}`}>
                         <DatabaseZap className={`w-8 h-8 sm:w-12 sm:h-12 transition-colors duration-500 ${step >= 3 ? 'text-blue-400' : 'text-slate-700'}`} />
                         {step === 4 && (
    ...
    return (
        <SimulationStage>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-12 w-full px-4 sm:px-8 py-4">
                {/* Model Hardware */}
                <div className={`w-36 h-48 sm:w-52 sm:h-64 border-2 rounded-[1.5rem] sm:rounded-[2rem] bg-slate-900 flex flex-col items-center justify-center p-4 sm:p-6 transition-all duration-1000 relative overflow-hidden ${cycle === 0 ? 'border-blue-500 shadow-[0_0_30px_rgba(59,130,246,0.15)]' : 'border-slate-800'}`}>
                    <div className="relative mb-3 sm:mb-6">
                        <Cpu className={`w-6 h-6 sm:w-10 sm:h-10 transition-colors duration-500 ${cycle >= 1 ? 'text-slate-700' : 'text-blue-400'}`} />
                        {cycle >= 3 && <Sparkles className="absolute -top-3 -right-3 w-4 h-4 text-blue-400 animate-pulse" />}
                    </div>
                ...
                {/* Telemetry Panel */}
                <div className={`w-full sm:w-72 h-44 sm:h-64 border-2 rounded-2xl sm:rounded-3xl bg-slate-950 p-4 sm:p-6 flex flex-col justify-between transition-all duration-1000 ${cycle === 4 ? 'border-green-500 shadow-[0_0_40px_rgba(34,197,94,0.08)]' : 'border-slate-800'}`}>
                    <div className="flex justify-between items-start">
                        ...
                        <Terminal className={`w-3 h-3 sm:w-5 sm:h-5 ${cycle >= 1 ? 'text-tva-orange' : 'text-slate-800'}`} />
                    </div>
                    
                    <div className="flex-1 flex items-end gap-1 mt-2 sm:mt-6 border-b border-white/5 pb-1 h-12 sm:h-20 overflow-hidden">
                        {Array.from({length: 10}).map((_, i) => (
                            <div key={i} className={`flex-1 rounded-t-sm transition-all duration-1000 ${cycle >= 1 ? 'bg-blue-500/30' : 'bg-slate-900'}`} style={{ height: cycle === 0 ? '25%' : `${20 + Math.random() * (cycle * 20)}%` }}></div>
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:gap-4 mt-2 sm:mt-6">
                        ...
                        <div className="bg-slate-900 rounded-lg p-1 sm:p-2 border border-white/5 text-center">
                            ...
                            <div className="text-[9px] sm:text-[11px] font-mono font-bold text-blue-400">{cycle >= 3 ? '14ms' : '---'}</div>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    {nodeItems.map((item, i) => {
                        ...
                        return (
                            <div key={i} className="flex flex-col items-center gap-2 relative z-10 w-12 sm:w-16">
                                <div className={`w-9 h-9 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl border flex items-center justify-center transition-all duration-700 bg-slate-950 ${isActive ? 'border-tva-orange shadow-[0_0_30px_rgba(220,38,38,0.2)] scale-110' : 'border-slate-800'}`}>
                                    {/* Fix: casting to React.ReactElement<any> to avoid className/size property error */}
                                    {React.cloneElement(item.icon as React.ReactElement<any>, { className: isActive ? 'text-tva-orange' : 'text-slate-700', size: 18 })}
                                </div>
                    ...
                    <div className="flex-1 space-y-1 sm:space-y-2">
                        {[
                            ...
                            <div key={i} className={`flex gap-2 items-center transition-opacity duration-500 ${activeNode === i ? 'opacity-100' : 'opacity-5'}`}>
                                ...
                                <span className="text-blue-400 text-[7px] font-black">{log}</span>
                            </div>
                        ))}
                    </div>
                </div>
      ...
      <div className="w-full h-full flex items-center justify-center p-2 sm:p-4">
          ...
          <div className="absolute bottom-4 right-4 sm:bottom-6 sm:right-6 w-48 sm:w-56 bg-slate-900/95 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-white/10 shadow-2xl backdrop-blur-md">
              <div className="flex items-center gap-2 mb-2">
                  <Network size={12} className="text-tva-orange" />
                  <div className="text-[8px] sm:text-[9px] font-mono text-tva-orange font-bold uppercase tracking-widest">SUBGRAPH_SYNC</div>
              </div>
...
export const TechArchitecture: React.FC = () => {
  ...
  return (
    <section className="py-12 md:py-24 bg-white border-y border-gray-100 relative overflow-hidden" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
      <div className="container mx-auto px-6 relative z-10">
        <div className="text-center max-w-2xl mx-auto mb-8 sm:mb-12">
            <h2 className="text-3xl md:text-5xl font-sans font-bold text-gray-900 mb-4 tracking-tight">AI <span className="text-tva-orange">Architectures</span></h2>
            <p className="text-sm sm:text-base text-gray-500 font-light leading-relaxed">Hardened patterns tailored for enterprise security.</p>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-6 md:gap-8 items-stretch max-w-7xl mx-auto">
            <div className="lg:col-span-8 order-2 lg:order-1">
                <div className="bg-slate-950 rounded-2xl shadow-2xl overflow-hidden border border-slate-900 flex flex-col relative h-[500px] sm:h-[550px] lg:h-full lg:min-h-[600px]">
                    <div className="bg-slate-900/95 p-3 sm:p-4 border-b border-white/5 flex justify-between items-center z-20 shrink-0">
                         ...
                         <div className="font-mono text-[8px] sm:text-[9px] text-slate-500 uppercase font-black tracking-widest flex items-center gap-2"><Activity size={12} className="text-blue-500" /> SYSTEM_STREAM // {activeTab}</div>
                    </div>
            ...
            <div className="lg:col-span-4 flex flex-col gap-3 order-1 lg:order-2">
                {architectureTabs.map((key) => {
                    ...
                    return (
                        <button key={key} onClick={() => { setActiveTab(key); setProgress(0); }} className={`w-full text-left p-4 rounded-xl border transition-all duration-500 relative ${isActive ? 'bg-white border-tva-orange shadow-lg lg:scale-[1.02]' : 'bg-white border-gray-100 hover:border-gray-200'}`}>
                            {isActive && <div className="absolute bottom-0 left-4 right-4 h-0.5 bg-gray-50 overflow-hidden"><div className="h-full bg-tva-orange" style={{ width: `${progress}%` }}></div></div>}
                            <div className="flex items-center sm:items-start gap-3">
                                <span className={`p-2 rounded-lg transition-all shrink-0 ${isActive ? 'bg-tva-orange text-white shadow-lg' : 'bg-gray-50 text-gray-400'}`}>{data.icon}</span>
                                <div className="min-w-0">
                                    <h3 className={`font-bold text-xs sm:text-sm mb-0.5 truncate ${isActive ? 'text-gray-900' : 'text-gray-500'}`}>{data.title}</h3>
                                    <p className="text-[9px] sm:text-[10px] text-gray-400 font-medium leading-tight line-clamp-2">{data.desc}</p>
                                </div>
                            </div>
                        </button>
                    );
                })}
                <div className="mt-auto p-4 bg-slate-50 rounded-xl border border-slate-100 border-dashed flex items-center gap-3">
                    <ShieldCheck className="text-tva-orange w-5 h-5 shrink-0" />
                    <p className="text-[9px] text-gray-500 leading-tight font-black uppercase tracking-widest">SOC2 TYPE II INFRASTRUCTURE</p>
                </div>
            </div>
        </div>
      </div>
